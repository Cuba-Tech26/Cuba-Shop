...
export const loginUser = async (req, res, next) => {
  ...
  try {
    ...
    if (!userExists) return next(customError(400, "User does not exists"));
...
export const getSingleUser = async(req, res, next) => {
  ...
  try {
    ...
    if (!user) return next(customError(400, "Can't find user"))
...
export const updateUser = async(req, res) => {
  ...
  try {
    if(userId) {
      ...
      // const {passsword, ...others} = updatedUser._doc
